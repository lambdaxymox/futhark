...
replaceStaticValSizes substs sv =
  case sv of
    LambdaSV sizes param t e closure_env ->
      LambdaSV
        (nubOrd $ mapMaybe onSizeParam sizes)
        (onAST param)
        (onType t)
        (onExtExp e)
        (onEnv closure_env)
    ...
    IntrinsicSV ->
      IntrinsicSV
  where
    onName v = case M.lookup v substs of
    ...
    onEnv =
      M.fromList
        . map (bimap onName onBinding)
        . M.toList

    onBinding (Binding t bsv) =
      Binding (second onType <$> t) (replaceStaticValSizes substs bsv)
...
defuncExp (LetWith id1 id2 idxs e1 body t loc) = do
  e1' <- defuncExp' e1
  sv1 <-
    lookupVar (toStruct (unInfo (identType id2))) (identSrcLoc id2) $
      identName id2
  idxs' <- mapM defuncDimIndex idxs
  (body', sv) <-
    localEnv (M.singleton (identName id1) (Binding Nothing sv1)) $
      defuncExp body
      ...
      when False $
        traceM $
          unlines
            [ "PARAM " ++ pretty pat,
              "SIZES " ++ pretty dims,
              "F TYPE " ++ pretty (typeOf e1),
              "ARG TYPE " ++ pretty (typeOf e2),
              "PARAM WITH ARG " ++ pretty pat'
            ]
